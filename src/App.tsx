...
export default function App() {
  ...
  return (
    <div
      ...
      className="min-h-screen bg-artistic-bg text-artistic-text font-sans selection:bg-artistic-cream-light selection:text-artistic-text antialiased overflow-x-hidden"
    >
          ...
          >
            {guestName && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-white/60 backdrop-blur-md border border-artistic-border px-4 py-1.5 rounded-sm shadow-xs text-[9px] uppercase font-sans tracking-[0.2em] text-artistic-gold font-bold">
                Spesial untuk:{" "}
                <span className="font-extrabold text-[#332F2C]">
                  {guestName}
                </span>
              </div>
            )}
